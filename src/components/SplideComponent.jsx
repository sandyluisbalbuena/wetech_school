...
export default function SplideComponent() {
	...
	return (
		...
		>
			{navs.map((nav, index)=>(
				<SplideSlide key={index} className='hover:cursor-pointer bg-transparent'>
					<div  onClick={ ()=>handleNavClick(nav[1]) } className={`animate__animated animate__fadeIn relative shadow-md shadow-secondary m-2 lg:m-5 bg-primary hover:scale-105 rounded-md transition-transform duration-200 px-5`}>
